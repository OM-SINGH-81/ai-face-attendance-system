...
# ===== GLOBAL FLAG TO CONTROL LIVE ATTENDANCE =====
STOP_LIVE_ATTENDANCE = False
...
# 🚫 YOLO: Detect phone / screen to prevent proxy
def is_phone_or_screen_detected(frame):
    """
    Returns True if phone, laptop, tablet, or screen is detected
    """
    ...
    for r in results:
        for cls in r.boxes.cls:
            ...
            if label in ["cell phone", "laptop", "tv", "tablet"]:
                return True
...
def capture_and_recognize(request):
    ...
    def process_frame(cam_config, stop_event):
        ...
        try:
            ...
            while not stop_event.is_set() and not STOP_LIVE_ATTENDANCE:
                                    ...
                                    if students.exists():
                                        ...
                                        if created:
                                            ...
                                            if attendance.check_in_time and not attendance.check_out_time:
                                                if timezone.now() >= attendance.check_in_time + timedelta(seconds=30):
...
@login_required
def stop_live_attendance(request):
    global STOP_LIVE_ATTENDANCE
    STOP_LIVE_ATTENDANCE = True
    messages.success(request, "Live attendance stopped.")
    return redirect('home')



def home(request):
    global STOP_LIVE_ATTENDANCE   # 👈 ADD THIS
    ...
    context = {
        ...
        # 👇 STEP 3 (LIVE STATUS FLAG)
        'live_status': not STOP_LIVE_ATTENDANCE,
    }
